...
describe Admin::Requests::VolunteerNotifier do
  ...
  describe '#notify_assigned' do
    ...
    it 'notifies assigned volunteers via push if fcm is present and notifications are active' do
      ...
      expect(MessagingService).to receive(:create_and_send_message).with(direction: :outgoing,
    ...
    it 'does not notify assigned volunteers via push if fcm is present and notifications are disabled' do
      allow_any_instance_of(Volunteer).to receive(:fcm_active?).and_return(true)
    ...
    it 'notifies assigned volunteers via sms if fcm is not present' do
  ...
  describe '#notify_updated' do
    ...
    it 'does not notify assigned volunteers via push if volunteer rejected offer' do
      requested_volunteer.rejected!
      allow_any_instance_of(Volunteer).to receive(:fcm_active?).and_return(true)
      allow_any_instance_of(Volunteer).to receive(:push_notifications?).and_return(true)
      expect(MessagingService).not_to receive(:create_and_send_message)
    ...
    it 'does not notify assigned volunteers via sms if fcm is not present' do
      allow_any_instance_of(Volunteer).to receive(:fcm_active?).and_return(false)
      allow_any_instance_of(Volunteer).to receive(:push_notifications?).and_return(false)
      expect(MessagingService).not_to receive(:create_and_send_message)
